...
export function HeavensHomeView({ data }: { data: HomeDashboardData }) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {((metrics.pending_cancellations || 0) > 0 || (metrics.overdue_orders || 0) > 0) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(metrics.pending_cancellations || 0) > 0 && (
                        <div
                            className="bg-rose-50 border border-rose-100 rounded-xl p-4 flex items-center justify-between shadow-sm cursor-pointer hover:bg-rose-100 transition-colors"
                            onClick={() => router.push('/pedidos?filter=cancelacion_pendiente')}
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 className="font-bold text-rose-800">Solicitudes Pendientes</h4>
                                    <p className="text-sm text-rose-600">{metrics.pending_cancellations} cancelaciones por revisar</p>
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-amber-600">{metrics.overdue_orders} pedidos con pago vencido</p>
                                </div>
                            </div>
                        </div>
                    )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard
                    ...
                    value={metrics.pending_quality_reports || 0}
                    icon={PackageCheck}
                    color="amber"
                    description="Pendientes exportador"
                    onClick={() => router.push('/dashboard-nacionales')}
                />
            </div>

            {/* Content Layout */}
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                {/* Left Column (2/3) - Metrics & Operations */}
